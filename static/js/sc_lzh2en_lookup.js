...
sc.lzh2enLookup = {
    ...
    lookupWord: function(graph){
        ...
        if (sc.lzh2enData[graph])
        {
            ...
            return ('<tr><td class="ideograph"><a title="Go to full entry at the DDB. Login with user name ‘guest’" href="' + href + '">' + graph + '</a></td> <td class="meaning"> ' + sc.lzh2enData[graph][0] + ': ' + sc.lzh2enData[graph][1] + '</td></tr>');
        } else if (sc.lzh2enFallbackData[graph]) {
            var href = "http://www.buddhism-dict.net/cgi-bin/dealt-lookup?q=" + encodeURI(graph);
            return ('<tr class="fallback"><td class="ideograph"><a title="Go to full entry at the DDB. Login with user name ‘guest’" href="' + href + '">' + graph + '</a></td> <td class="meaning"> ' + sc.lzh2enFallbackData[graph][0] + ': ' + sc.lzh2enFallbackData[graph][1] + '</td></tr>')
        }
        return "";
    },
